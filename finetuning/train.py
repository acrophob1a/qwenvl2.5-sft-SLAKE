...
def train(attn_implementation="flash_attention_2"):
    ...
    def smart_add_special_tokens(new_tokens):
        num_added = tokenizer.add_special_tokens({'additional_special_tokens': new_tokens})
        model.resize_token_embeddings(len(tokenizer))
        if is_main_process():
            print(f"Added {num_added} special tokens") 
    
    if cfg.add_grounding_specific_tokens:
        # breakpoint() # model.get_input_embeddings().weight.shape[0], model.get_output_embeddings().weight.shape[0], len(tokenizer)
        new_tokens = [f'<{i}>' for i in range(1000)]
        smart_add_special_tokens(new_tokens)
        # breakpoint() # model.get_input_embeddings().weight.shape[0], model.get_output_embeddings().weight.shape[0], len(tokenizer)
    ...
    # breakpoint() # train_dataset[0]['input_ids']
